...
namespace Adagio
{
	...
	: m_AudioState(PlayState::NotStarted)
	{
	...
	void Application::ProcessCommands()
	{
		...
		while (CommandQueue::GetInstance().Pop(cmd))
		{
			...
			case CommandType::Play:
				...
				MessageQueue::GetInstance().Push("{\"type\":\"info\",\"value\":\"Playback started\"}");
				break;
			case CommandType::Pause:
				UpdateAudioState(PlayState::Paused);
				MessageQueue::GetInstance().Push("{\"type\":\"info\",\"value\":\"Playback paused\"}");
				break;
			case CommandType::Stop:
				UpdateAudioState(PlayState::Stopped);
				MessageQueue::GetInstance().Push("{\"type\":\"info\",\"value\":\"Playback stopped\"}");
				break;
			case CommandType::Load:
				try
				{
					...
					if (success == -1)
						MessageQueue::GetInstance().Push("{\"type\":\"error\",\"value\":\"Failed to load audio file.\"}");
					else if (success == 1)
						MessageQueue::GetInstance().Push("{\"type\":\"info\",\"value\":\"Audio file loaded successfully.\"}");
			...
			case CommandType::Clear:
				ClearAudio();
				MessageQueue::GetInstance().Push("{\"type\":\"fileClosed\"}");
				break;
	...
	int Application::LoadAudio(std::string filePath)
	{
		...
		if (m_AudioData != nullptr)
		{
			...
			MessageQueue::GetInstance().Push("{\"type\":\"fileLoaded\",\"value\":{\"duration\":" + std::to_string(m_AudioData->Duration) + "} }");
			waveformThread.join();
		}
		return 1;
	}
